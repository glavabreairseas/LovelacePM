...
def aicm_norm_conv(aicm3, nvectmat):
    return (
        (aicm3[0, :, :].T * nvectmat[:, 0]).T
        + (aicm3[1, :, :].T * nvectmat[:, 1]).T
        + (aicm3[2, :, :].T * nvectmat[:, 2]).T
    )


def isitin(
    pts, pcont, centers, nvect, isvalid, tolerance, npan
):  # the function capable of ruining any man's pride!
    ...
    for i in range(4):
        ...
        dot[isin] = (
            prod[isin, 0] * nvect[isin, 0]
            + prod[isin, 1] * nvect[isin, 1]
            + prod[isin, 2] * nvect[isin, 2]
        )
        isin[isin] = np.logical_and(
            isin[isin],
            np.logical_or(
                dot[isin] < tolerance, lg.norm(side[isin, :], axis=1) < tolerance
            ),
        )
    return isin
...
def get_panel_contact(p, u, centers, nvects, pts, tolerance):
    ...
    pnorm = (
        (p[0] - centers[:, 0]) * nvects[:, 0]
        + (p[1] - centers[:, 1]) * nvects[:, 1]
        + (p[2] - centers[:, 2]) * nvects[:, 2]
    )
    unorm = (u @ nvects.T).T
