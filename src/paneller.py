...
def subprocess_genaicm(
    queue1, queue2
):  # generic function to unpack AICM calc order from multiprocessing
...
class WakeLine:  # class encompassing info about a wake line.
    # Made so as to compute deformation with wake rollup
...
class Solid:
    def __init__(
        self, sldlist=[], wraparounds=[], full_parallel=False
    ):  # solid data type
    ...
    def addpatch(
        self, sld, wraps=[], prevlines={}, invlats=[], tolerance=0.00005
    ):  # add panel based on point grid (list of lists)
        # returns: arrays of, respectively, horizontal and vertical
        # line indexes; panel indexes list of lists;
        # point grid inserted as input, for external reference from
        # high-end functions
        # lines provided with -2 index in prevlines will be accounted
        # as new lines, and as -1,
        ...
        for i in range(len(sld) - 1):
            ...
            for j in range(len(sld[0]) - 1):
                if i == 0 and "low" in prevlines:
                    ...
                    horzlines[-1] += [
                        self.addline(
                            np.vstack((sld[i][j], sld[i][j + 1])).T, tolerance=tolerance
                        )
                    ]
                if j == 0 and "right" in prevlines:
                    ...
                    vertlines[-1] += [
                        self.addline(
                            np.vstack((sld[i][j], sld[i + 1][j])).T, tolerance=tolerance
                        )
                    ]
        if 0 in wraps:  # if wrapped in x direction (inner list layer),
            # repeat the first lateral vertical lines
            for i in range(len(sld) - 1):
    ...
    def addpanel(self, lines, invs=[]):  # line list: includes indexes summed by one,
        # inverted to indicate oposite direction
    ...
    def addwakepanel(
        ...
        indup=0,
        indown=2,
        offsetleft=np.array([1000.0, 0.0, 0.0]),
        offsetright=np.array([]),
        tolerance=0.00005,
        disc=10,
        ...
        # add line segments correspondent to panel wake, and remove
        # "inup-th"/"indown-th" line segment in the panel's list
        ...
        if indup < len(self.panels[refup].lines):
            ...
            te_linind = self.panels[refup].wakelines.pop(
                indup - len(self.panels[refup].lines)
            )
        ...
        for line in leftlines + rightlines:
            self.wakelines.add(line)
        for lind in indsup:
            self.wakeline_inds.add(abs(lind) - 1)
        if refdown != -1:
            if indown < len(self.panels[refdown].lines):
                te_linind = self.panels[refdown].lines.pop(indown)
                self.panels[refdown].wakelines += [-lind for lind in indsup]
                self.panels[refdown].TE_line = te_linind
        for i in range(disc):
            center = (
                self.line_midpoint(rightlines[i].ind)
                + self.line_midpoint(leftlines[disc - i - 1].ind)
            ) / 2
            self.wakestrips[-1] += [
                WakePanel(leftlines[disc - i - 1], rightlines[i], center)
            ]
    ...
    def genwakepanels(
        ...
        # wakeinds: list of lists indicating corresponding wake vortex line
        # segment index to apply kutta condition to
        if len(wakeinds) == 0:
            wakeinds = [[0, 2]]
        trimlist(len(wakecombs), wakeinds)
        self.addto += [[wakecombs[0][0], wakecombs[0][1]]]
        left, prevleft = self.addwakepanel(
            wakecombs[0][0],
            wakecombs[0][1],
            indup=wakeinds[0][0],
            indown=wakeinds[0][0],
        ...
        for i in range(1, len(wakecombs) - 1):
            ...
            _, prevleft = self.addwakepanel(
                wakecombs[i][0],
                wakecombs[i][1],
                indup=wakeinds[i][0],
                indown=wakeinds[i][0],
                offsetleft=np.array([cos(a) * cos(b), -cos(a) * sin(b), sin(a)])
                * offset,
                disc=disc,
                strategy=strategy,
                prevleft=prevleft,
    ...
    def panel_getcoords(
        self, p
    ):  # get coordinates for panel based on line vectors and lists
        coords = np.zeros((3, 4))
        for i in range(len(p.lines)):
            coords[:, i] = self.lines[
                abs(p.lines[i]) - 1, :, (1 if p.lines[i] > 0 else 0)
            ]
        if len(p.lines) < 4:
            coords[:, -1] = coords[:, -2]
        return coords
    ...
    def panel_calcSn(self, p):  # update panel area and normal vector
        coord = self.line_getcoords(p.lines[0])
        p.nvector = np.cross(coord[:, 1] - p.colpoint, coord[:, 0] - p.colpoint)
        p.S = lg.norm(p.nvector)
        p.nvector /= p.S
        p.S /= 2
        for i in range(1, len(p.lines)):
            coord = self.line_getcoords(p.lines[i])
            p.S += (
                lg.norm(np.cross(coord[:, 1] - p.colpoint, coord[:, 0] - p.colpoint))
                / 2
            )

    def line_getcoords(
        self, ind
    ):  # return line coordinates in order presented in panel,
        # based on panel.lines element
        if ind > 0:
    ...
    def nvect_diradjust(
        self, patchinds, vect
    ):  # adjust normal vectors in patch to follow a certain direction
    ...
    def nvect_radadjust(
        self, patchinds, center, inwards=False
    ):  # adjust normal vectors in patch to follow radial convergent or
        # divergent distribution
        if inwards:
            for patchlist in patchinds:
                for i in patchlist:
                    if (
                        self.panels[i].nvector @ (self.panels[i].colpoint - center)
                        > 0.0
                    ):
                        self.panels[i].nvector *= -1
        else:
    ...
    def lineadjust(
        self, patchinds=[]
    ):  # adjust line fortran index signals to comply with dextrogyre panel convention
        if len(patchinds) == 0:
        ...
        for patchlist in patchinds:
            for i in patchlist:
                for lind in range(len(self.panels[i].lines)):
                    p = (
                        self.lines[abs(self.panels[i].lines[lind]) - 1, :, 0]
                        - self.panels[i].colpoint
                    )
                    u = (
                        self.lines[abs(self.panels[i].lines[lind]) - 1, :, 1]
                        - self.lines[abs(self.panels[i].lines[lind]) - 1, :, 0]
                    )
                    if (
                        u
                        @ np.cross(self.panels[i].nvector, p)
                        * self.panels[i].lines[lind]
                        > 0.0
                    ):
                        self.panels[i].lines[lind] *= -1
                        print(
                            "WARNING: line "
                            + str(abs(self.panels[i].lines[lind]) - 1)
                            + " in panel "
                            + str(i)
                            + " had to be inverted. Please check integrity "
                            + "of patchcompose() functions"
                        )
                        self.problematic += [
                            abs(line) - 1 for line in self.panels[i].lines
                        ]

    def iscontiguous(
        self, patchinds=[], tolerance=5e-5
    ):  # check if all panels in solid are closed quadrilaterals
        if len(patchinds) == 0:
            patchinds = [list(range(self.npanels))]
        for patchlist in patchinds:
            for i in patchlist:
    ...
    def end_preprocess(self, paninds=[], tolerance=5e-5, initialize=True, check=True):
        # calculate panel areas before they are altered by wake generation.
        # Must be run before it, and terrible
        # consequences may arise if done otherwise
        u = np.array([0.0, 0.0, 0.0])
        v = np.array([0.0, 0.0, 0.0])
        # analyse whether provided panel index list corresponds to a certain set or,
        # as default, indicates all panels in self
        ...
        for p in panlist:
            ...
            if len(p.lines) == 3:
                ...
                p.colpoint = (
                    self.line_midpoint(p.lines[0])
                    + self.line_midpoint(p.lines[1])
                    + self.line_midpoint(p.lines[2])
                ) / 3
            elif len(p.lines) == 4:
                p.S = (
                    lg.norm(p.nvector)
                    + lg.norm(
                        np.cross(
                            self.line_getvec(p.lines[2]), self.line_getvec(p.lines[3])
                        )
                    )
                ) / 2
                p.nvector /= lg.norm(p.nvector)
                p.colpoint = (
                    ...
                    + self.line_midpoint(p.lines[1])
                    + self.line_midpoint(p.lines[2])
                    + self.line_midpoint(p.lines[3])
                ) / 4
            else:
                p.nvector = np.array([0.0, 0.0, 0.0])
                p.colpoint = (
                    self.line_midpoint(p.lines[0]) + self.line_midpoint(p.lines[1])
                ) / 2
                p.S = 0.0
            if p.S <= tolerance**2:
                print(
        ...
        if initialize:
            ...
            self.forces = []
            self.moments = []
        # adjust lines in case any is set inconsistently with respect
        # to anti-clockwise convention in panel
        if check:
            self.lineadjust()
    ...
    def addorder(
        self, queue, colmat, ind1, ind2
    ):  # add order to queue for subprocess to compute AICM from rows ind1 to ind2
    ...
    def gen_farfield(
        self, Uinf, a=0.0, b=0.0, p=0.0, q=0.0, r=0.0
    ):  # generate generic local freestream velocity dependant on parameters
        newvec = np.zeros((self.npanels, 3))
        for i in range(self.npanels):
            newvec[i, :] = Uinf * np.array(
                [cos(a) * cos(b), -cos(a) * sin(b), sin(a)], dtype="double"
            ) + np.cross(np.array([p, -q, r], dtype="double"), self.panels[i].colpoint)
        return newvec

    def gen_wake_farfield(
        self, Uinf, a=0.0, b=0.0, p=0.0, q=0.0, r=0.0
    ):  # generate generic local freestream at wake panels
        for strip in self.wakestrips:
            for pan in strip:
                pan.v = Uinf * np.array(
                    [cos(a) * cos(b), -cos(a) * sin(b), sin(a)], dtype="double"
                ) + np.cross(np.array([p, -q, r], dtype="double"), pan.center)

    def gen_farfield_derivative(
        self, Uinf, a=0.0, b=0.0, p=0.0, q=0.0, r=0.0, par="a"
    ):  # generate generic local freestream velocity dependant on parameters
        newvec = np.zeros((self.npanels, 3))
        if par == "a":
            for i in range(self.npanels):
                newvec[i, :] = Uinf * np.array(
                    [-sin(a) * cos(b), sin(a) * sin(b), cos(a)], dtype="double"
                )
        elif par == "b":
            for i in range(self.npanels):
                newvec[i, :] = Uinf * np.array(
                    [-cos(a) * sin(b), -cos(a) * cos(b), 0.0], dtype="double"
                )
        elif par == "p":
            for i in range(self.npanels):
                newvec[i, :] = np.cross(
                    np.array([1.0, 0.0, 0.0], dtype="double"), self.panels[i].colpoint
                )
        elif par == "q":
            for i in range(self.npanels):
                newvec[i, :] = np.cross(
                    np.array([0.0, -1.0, 0.0], dtype="double"), self.panels[i].colpoint
                )
        elif par == "r":
            for i in range(self.npanels):
                newvec[i, :] = np.cross(
                    np.array([0.0, 0.0, 1.0], dtype="double"), self.panels[i].colpoint
                )
        elif par == "Uinf":
            for i in range(self.npanels):
                newvec[i, :] = np.array(
    ...
    def genvbar(
        self, Uinf, a=0.0, b=0.0, p=0.0, q=0.0, r=0.0
    ):  # generate freestream velocity vector. Angular velocities
        # are raw (rad/s), not normalized by Uinf or dimensions
    ...
    def gen_selfinf_mat(
        self,
    ):  # generate self-influence matrix according to Srivastava's equations,
        # with each line's vorticity as an input
        ...
        for i in range(self.npanels):
            linelist = []
            nocirc_linelist = []
            for line in self.panels[i].lines:
                if not int(abs(line)) - 1 in self.panels[i].no_selfinf:
                    linelist += [int(abs(line)) - 1]
                else:
                    nocirc_linelist += [int(abs(line)) - 1]
            for line in linelist:
                vdv = np.cross(
                    self.lines[line, :, 1] - self.lines[line, :, 0],
                    self.panels[i].nvector,
                ) / (4 * self.panels[i].S)
                self.selfinf_mat_x[i, line] = vdv[0]
                self.selfinf_mat_y[i, line] = vdv[1]
                self.selfinf_mat_z[i, line] = vdv[2]
            for line in nocirc_linelist:
                vdv = np.cross(
                    self.lines[line, :, 1] - self.lines[line, :, 0],
                    self.panels[i].nvector,
                ) / (4 * self.panels[i].S)
    ...
    def gen_selfinf(
        self,
    ):  # generate self-influence velocity according to Srivastava's equations
    ...
    def solve(
        ...
        # generate Euler solution. Inverts AIC matrix with
        # Tikhonov regularization if "damper" is set to non-zero value.
        # self.iaicm=toolkit.aicm_inversion(self.aicm, damper)
        if len(target) == 0:
            target = np.zeros(
                self.npanels
            )  # target: nvv aimed at, for stability derivative computation
            # and/or viscous-inviscid coupling

        if damper != 0.0:
            self.iaicm = (
                slg.inv(
                    self.aicm.T @ self.aicm
                    + damper * np.eye(self.npanels, self.npanels)
                )
                @ self.aicm.T
            )
        else:
        ...
        for i in range(wakeiter):  # iterate for wake generation
            ...
            self.wake_rollup()
            trol = tm.time()
            self.aicm3_line[:, :, wakelininds] = pytoolkit.aicm_lines_recalc(
                wakelininds, self.lines, colmat
            )
            for j in range(3):
            ...
            print(
                "%14s %3d | %5f %5f %5f"
                % ("Wake teration", i, tvel - t0, trol - tvel, tsol - trol)
            )
    ...
    def velcompute(self):
        for i in range(3):
            self.delphi[:, i] = (
                self.aicm3[i, :, :] @ self.solution
            )  # compute local velocity due to disturbance field
        self.gen_selfinf()
        ...
        if M == 0.0:
            ...
            for i in range(self.npanels):
                incomp = (
                    ...
                    - (self.delphi[i, :] + self.vbar[i, :])
                    @ (self.delphi[i, :] + self.vbar[i, :])
                ) / Uinf**2  # incompressible value
                self.Cps[i] = (
                    2
                    * (
                        (1.0 + (gamma - 1) * M**2 * incomp / 2) ** (gamma / (gamma - 1))
                        - 1
                    )
                    / (M**2 * gamma)
                )
    ...
    def calcforces(
        self,
    ):  # compute force correspondent to unitary dynamic pressure on each panel
        self.forces = [
            -self.panels[i].S * self.panels[i].nvector * self.Cps[i]
            + self.panels[i].S
            * self.Cfs[i]
            * (self.vbar[i, :] + self.delphi[i, :])
            / lg.norm(self.vbar[i, :] + self.delphi[i, :])
            for i in range(self.npanels)
        ]
        self.moments = [
            np.cross(self.panels[i].colpoint, self.forces[i])
            for i in range(self.npanels)
        ]

    def calc_derivative_dv(
        self, Uinf, dvdksi, M=0.0, gamma=1.4
    ):  # calculate local Cp derivative by freestream derivative
        dndksi = np.array(
            [self.panels[i].nvector @ dvdksi[i, :] for i in range(self.npanels)]
        )
        dGammadksi = -self.iaicm @ dndksi
        dGamma_linedksi = self.panline_matrix @ dGammadksi
        dvdksi[:, 0] += (
            self.aicm3[0, :, :] @ dGammadksi + self.selfinf_mat_x @ dGamma_linedksi
        )
        dvdksi[:, 1] += (
            self.aicm3[1, :, :] @ dGammadksi + self.selfinf_mat_y @ dGamma_linedksi
        )
        dvdksi[:, 2] += (
            self.aicm3[2, :, :] @ dGammadksi + self.selfinf_mat_z @ dGamma_linedksi
        )
        dCps = np.array(
            [
                -(2 * (self.vbar[i, :] + self.delphi[i, :]) @ dvdksi[i, :]) / Uinf**2
                for i in range(self.npanels)
            ]
        )
        if M != 0.0:
            for i in range(self.npanels):
                ...
                dCps[i] = (
                    2
                    * ((1 + (gamma - 1) * M**2 * incomp / 2) ** (1.0 / (gamma - 1.0)))
                    * dCps[i]
                    / (M**2 * (gamma - 1.0))
                )
        return dCps

    def calc_derivative_dn(
        self, Uinf, dndksi, M=0.0, gamma=1.4
    ):  # calculate local Cp derivative by normal velocity derivative
        dGammadksi = -self.iaicm @ dndksi
        dGamma_linedksi = self.panline_matrix @ dGammadksi
    ...
    def add_wakevels(
        self, tolerance=1e-5
    ):  # calculate velocities at wake panel control points
        # and add to local velocity variables
        for strip in self.wakestrips:
            for p in strip:
                p.v += pytoolkit.get_field_influence(
                    self.lines, self.solution_line, p.center, tolerance=tolerance
                )

    def wake_redef(
        self, Uinf=1.0, a=0.0, b=0.0
    ):  # redefine wake positions based on new freestream parameters
        self.gen_wake_farfield(Uinf=Uinf, a=a, b=b, p=0.0, q=0.0, r=0.0)
        self.wake_rollup()

    def wake_rollup(
        self,
    ):  # calculate velocity at nodes located at wake line midpoints
    ...
    def wakelines_deform(
        self,
    ):  # deform wake lines according to computed velocity
        # at panels between them
        for line in self.wakelines:
            line.v /= line.nabut
            line.inverted = (
                self.lines[abs(line.ind) - 1, 0, 0]
                > self.lines[abs(line.ind) - 1, 0, 1]
            )
            line.updateme = True
        deltaxl = 0.0
        deltaxr = 0.0
        ...
        newright = np.zeros(3)
        for strip in self.wakestrips:
            origin_left = self.lines[
                abs(strip[0].wl.ind) - 1, :, 1 if strip[0].wl.inverted else 0
            ]
            origin_right = self.lines[
                abs(strip[0].wr.ind) - 1, :, 1 if strip[0].wr.inverted else 0
            ]
            for i in range(len(strip)):
                if strip[i].wl.updateme:
                    deltaxl = abs(
                        self.lines[abs(strip[i].wl.ind) - 1, 0, 1]
                        - self.lines[abs(strip[i].wl.ind) - 1, 0, 0]
                    )
                ...
                if strip[i].wr.updateme:
                    deltaxr = abs(
                        self.lines[abs(strip[i].wr.ind) - 1, 0, 1]
                        - self.lines[abs(strip[i].wr.ind) - 1, 0, 0]
                    )
                ...
                strip[i].center = (
                    self.line_midpoint(strip[i].wr.ind)
                    + self.line_midpoint(strip[i].wl.ind)
                ) / 2
        for line in self.wakelines:
            line.v = 0.0
            line.nabut = 0
    ...
    def PG_apply(
        self, beta, a, b
    ):  # beta indicates PG correction factor for compressibility
        PG_apmat = PG_xmult(beta, a, b)
        PG_apv = PG_unitov(beta, a, b)
        # convert lines
        self.lines[:, 0, :] = (
            PG_apmat[0, 0] * self.lines[:, 0, :]
            + PG_apmat[0, 1] * self.lines[:, 1, :]
            + PG_apmat[0, 2] * self.lines[:, 2, :]
        )
        self.lines[:, 1, :] = (
            PG_apmat[1, 0] * self.lines[:, 0, :]
            + PG_apmat[1, 1] * self.lines[:, 1, :]
            + PG_apmat[1, 2] * self.lines[:, 2, :]
        )
        self.lines[:, 2, :] = (
            PG_apmat[2, 0] * self.lines[:, 0, :]
            + PG_apmat[2, 1] * self.lines[:, 1, :]
            + PG_apmat[2, 2] * self.lines[:, 2, :]
        )
        # convert panels
        ...
        if hasattr(self, "selfinf_mat_x"):
            ...
            self.selfinf_mat_x = (
                PG_apv[0, 0] * self.selfinf_mat_x
                + PG_apv[0, 1] * self.selfinf_mat_y
                + PG_apv[0, 2] * self.selfinf_mat_z
            )
            self.selfinf_mat_y = (
                PG_apv[1, 0] * self.selfinf_mat_x
                + PG_apv[1, 1] * self.selfinf_mat_y
                + PG_apv[1, 2] * self.selfinf_mat_z
            )
            self.selfinf_mat_z = (
                PG_apv[2, 0] * self.selfinf_mat_x
                + PG_apv[2, 1] * self.selfinf_mat_y
                + PG_apv[2, 2] * self.selfinf_mat_z
            )
        if hasattr(self, "aicm3_line"):
            # self.aicm remains the same
            self.aicm3_line[0, :, :] = (
                PG_apv[0, 0] * self.aicm3_line[0, :, :]
                + PG_apv[0, 1] * self.aicm3_line[1, :, :]
                + PG_apv[0, 2] * self.aicm3_line[2, :, :]
            )
            self.aicm3_line[1, :, :] = (
                PG_apv[1, 0] * self.aicm3_line[0, :, :]
                + PG_apv[1, 1] * self.aicm3_line[1, :, :]
                + PG_apv[1, 2] * self.aicm3_line[2, :, :]
            )
            self.aicm3_line[2, :, :] = (
                PG_apv[2, 0] * self.aicm3_line[0, :, :]
                + PG_apv[2, 1] * self.aicm3_line[1, :, :]
                + PG_apv[2, 2] * self.aicm3_line[2, :, :]
            )
            self.aicm3[0, :, :] = (
                PG_apv[0, 0] * self.aicm3[0, :, :]
                + PG_apv[0, 1] * self.aicm3[1, :, :]
                + PG_apv[0, 2] * self.aicm3[2, :, :]
            )
            self.aicm3[1, :, :] = (
                PG_apv[1, 0] * self.aicm3[0, :, :]
                + PG_apv[1, 1] * self.aicm3[1, :, :]
                + PG_apv[1, 2] * self.aicm3[2, :, :]
            )
            self.aicm3[2, :, :] = (
                PG_apv[2, 0] * self.aicm3[0, :, :]
                + PG_apv[2, 1] * self.aicm3[1, :, :]
                + PG_apv[2, 2] * self.aicm3[2, :, :]
            )

    def PG_remove(
        self, beta, a, b
    ):  # remove effect of previously imposed PG correction
    ...
    def plotnormals(self, xlim=[], ylim=[], zlim=[], factor=1.0):
        if self.runme:
            # plot normal vectors of panels. Function essentially
            # produced for geometry gen. debugging
            ...
            ax.quiver(
                [p.colpoint[0] for p in self.panels],
                [p.colpoint[1] for p in self.panels],
                [p.colpoint[2] for p in self.panels],
                [p.nvector[0] * factor for p in self.panels],
    ...
    def resolve(
        ...
        wakeiter=0,
        tolerance=1e-5,
    ):  # Re-run a simulation redefining the wake to new  freestream properties
        if self.runme:
            if echo:
                print("========Euler solution=======")
                print(self.npanels, " panels, ", self.nlines, " lines")
    ...
    def aic_memory_clean(
        self,
    ):  # garbage-collect memory from AIC matrixes, which can be,
        # with this method, deleted after calculations
        if self.runme:
