...
def tstep_solve(
    ...
    moments = np.zeros(3)
    mtemp = np.zeros(3)
    # for pertutbation, here we use the notation employed by
    # Drela in Flight Viechle Aerodynamics
    # equations are also directly obtained from there
    # notation for coefs: (CX, CY, CZ, Cl, Cm, Cn)
    # notation for derivs: (CX, CY, CZ, Cl, Cm, Cn),
    # first row: a derivatives; second, b derivatives; third, p; 4, q; 5, r
    xe = perturbation[0]
    ye = perturbation[1]
    ze = perturbation[2]
    phi = perturbation[3]
        ...
        for j in range(3):
            forces[j] = (
                (
                    ...
                    + derivs[0, j] * a
                    + derivs[1, j] * b
                    + derivs[2, j] * p * bref / (2 * Uinf)
                    + derivs[3, j] * q * cref / (2 * Uinf)
                    + derivs[4, j] * r * bref / (2 * Uinf)
                )
                * Sref
                * Uinf**2
                * rho
            ...
            moments[j] = (
                (
                    ...
                    + derivs[0, j + 3] * a
                    + derivs[1, j + 3] * b
                    + derivs[2, j + 3] * p * bref / (2 * Uinf)
                    + derivs[3, j + 3] * q * cref / (2 * Uinf)
                    + derivs[4, j + 3] * r * bref / (2 * Uinf)
                )
                * Sref
                * (cref if j == 1 else bref)
                * Uinf**2
                * rho
                / 2
            )
