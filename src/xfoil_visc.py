...
""" Xfoil automation to temporarily supply the absense of a parallel,
estimative boundary layer solver
"""
...
def polar_data(
    ...
    # flap variable: [x_hinge, y_hinge, deflection(angles)].
    # aseq: same input as required for xfoil command
    ordir = os.getcwd()
    if len(afldir) == 0:
        os.chdir(ordir)
    else:
    ...
    if file_exists:
        ...
        if plat.system() == "Windows":
            xfoil.p = sub.Popen(
                ["xfoil.exe"], stdin=sub.PIPE, stdout=sub.DEVNULL, stderr=sub.DEVNULL
            )
        elif plat.system() == "Linux":
            xfoil.p = sub.Popen(
                ["xfoil"], stdin=sub.PIPE, stdout=sub.DEVNULL, stderr=sub.DEVNULL
            )
        xfoil.start()
        xfoil.join(30)
...
class polar_correction:
    def __init__(
        ...
        if (not full_parallel) and current_process().name == "LPM_child":
            ...
            alphas_Re_low1, Cls_Re_low1, Cds_Re_low1, Cms_Re_low1 = polar_data(
                name=name,
                afldir=afldir,
                ext_append=ext_append,
                aseq=[amed, aseq[1], aseq[2]],
                Re=Re_low,
                M=Mach,
                visc=True,
                iter=iter,
            )
            alphas_Re_high1, Cls_Re_high1, Cds_Re_high1, Cms_Re_high1 = polar_data(
                name=name,
                afldir=afldir,
                ext_append=ext_append,
                aseq=[amed, aseq[1], aseq[2]],
            ...
            alphas_Re_low2, Cls_Re_low2, Cds_Re_low2, Cms_Re_low2 = polar_data(
                name=name,
                afldir=afldir,
                ext_append=ext_append,
                aseq=[amed - aseq[2], aseq[0], -aseq[2]],
                Re=Re_low,
                M=Mach,
                visc=True,
                iter=iter,
            )
            alphas_Re_high2, Cls_Re_high2, Cds_Re_high2, Cms_Re_high2 = polar_data(
                name=name,
                afldir=afldir,
                ext_append=ext_append,
                aseq=[amed - aseq[2], aseq[0], -aseq[2]],
    ...
    def dump(
        self, poldir="", polname="n4412", ext_append=True, echo=True
    ):  # dump to file
        if self.runme:
            ordir = os.getcwd()
            if len(poldir) == 0:
            ...
            for i in range(len(self.alphas_inviscid)):
                file.write(
                    str(self.alphas_inviscid[i])
                    + " "
                    + str(self.Cls_inviscid[i])
                    + " "
                    + str(self.Cds_inviscid[i])
                    + " "
                    + str(self.Cms_inviscid[i])
                    + "\n"
                )
                if echo:
                    print(
            ...
            for i in range(len(self.alphas_Re_low)):
                file.write(
                    str(self.alphas_Re_low[i])
                    + " "
                    + str(self.Cls_Re_low[i])
                    + " "
                    + str(self.Cds_Re_low[i])
                    + " "
                    + str(self.Cms_Re_low[i])
                    + "\n"
                )
                if echo:
                    print(
            ...
            for i in range(len(self.alphas_Re_high)):
                file.write(
                    str(self.alphas_Re_high[i])
                    + " "
                    + str(self.Cls_Re_high[i])
                    + " "
                    + str(self.Cds_Re_high[i])
                    + " "
                    + str(self.Cms_Re_high[i])
                    + "\n"
                )
                if echo:
                    print(
    ...
    def create_functions(self, cubic=True):
        Cls_inviscid_Re_low = np.interp(
            self.alphas_Re_low, self.alphas_inviscid, self.Cls_inviscid
        )
        Cls_inviscid_Re_high = np.interp(
            self.alphas_Re_high, self.alphas_inviscid, self.Cls_inviscid
        )
        Cds_inviscid_Re_low = np.interp(
            self.alphas_Re_low, self.alphas_inviscid, self.Cds_inviscid
        )
        Cds_inviscid_Re_high = np.interp(
            self.alphas_Re_high, self.alphas_inviscid, self.Cds_inviscid
        )
        Cms_inviscid_Re_low = np.interp(
            self.alphas_Re_low, self.alphas_inviscid, self.Cms_inviscid
        )
        Cms_inviscid_Re_high = np.interp(
            self.alphas_Re_high, self.alphas_inviscid, self.Cms_inviscid
        )
        if cubic:
            self.alphas_fun = sinterp.CubicSpline(
                self.Cls_inviscid, self.alphas_inviscid
            )
            self.Cls_Re_low_fun = sinterp.CubicSpline(
                Cls_inviscid_Re_low, self.Cls_Re_low
            )
            self.Cls_Re_high_fun = sinterp.CubicSpline(
                Cls_inviscid_Re_high, self.Cls_Re_high
            )
            self.Cds_Re_low_fun = sinterp.CubicSpline(
                Cls_inviscid_Re_low, self.Cds_Re_low - Cds_inviscid_Re_low
            )
            self.Cds_Re_high_fun = sinterp.CubicSpline(
                Cls_inviscid_Re_high, self.Cds_Re_high - Cds_inviscid_Re_high
            )
            self.Cms_Re_low_fun = sinterp.CubicSpline(
                Cls_inviscid_Re_low, self.Cms_Re_low - Cms_inviscid_Re_low
            )
            self.Cms_Re_high_fun = sinterp.CubicSpline(
                Cls_inviscid_Re_high, self.Cms_Re_high - Cms_inviscid_Re_high
            )
        else:
            ...
            self.Cls_Re_high_fun = sinterp.interp1d(
                Cls_inviscid_Re_high, self.Cls_Re_high
            )
            self.Cds_Re_low_fun = sinterp.interp1d(
                Cls_inviscid_Re_low, self.Cds_Re_low - Cds_inviscid_Re_low
            )
            self.Cds_Re_high_fun = sinterp.interp1d(
                Cls_inviscid_Re_high, self.Cds_Re_high - Cds_inviscid_Re_high
            )
            self.Cms_Re_low_fun = sinterp.interp1d(
                Cls_inviscid_Re_low, self.Cms_Re_low - Cms_inviscid_Re_low
            )
            self.Cms_Re_high_fun = sinterp.interp1d(
                Cls_inviscid_Re_high, self.Cms_Re_high - Cms_inviscid_Re_high
            )

    def __call__(
        self, Re=2e6, inverse=False
    ):  # return fitting functions for a given Reynolds number
        if self.runme:
            eta = np.interp(
                Re, np.array([self.Re_low, self.Re_high]), np.array([0.0, 1.0])
            )
            if not inverse:
                alphas = lambda Cl: self.alphas_fun(-Cl)  # noqa E731
                Cls = lambda Cl: self.Cls_Re_low_fun(Cl) * (  # noqa E731
                    1.0 - eta
                ) + eta * self.Cls_Re_high_fun(Cl)
                Cds = lambda Cl: self.Cds_Re_low_fun(Cl) * (  # noqa E731
                    1.0 - eta
                ) + eta * self.Cds_Re_high_fun(Cl)
                Cms = lambda Cl: self.Cms_Re_low_fun(Cl) * (  # noqa E731
                    1.0 - eta
                ) + eta * self.Cms_Re_high_fun(Cl)
            else:
                alphas = lambda Cl: -self.alphas_fun(-Cl)  # noqa E731
                Cls = lambda Cl: -(  # noqa E731
                    self.Cls_Re_low_fun(-Cl) * (1.0 - eta)
                    + eta * self.Cls_Re_high_fun(-Cl)
                )
                Cds = lambda Cl: self.Cds_Re_low_fun(-Cl) * (  # noqa E731
                    1.0 - eta
                ) + eta * self.Cds_Re_high_fun(-Cl)
                Cms = lambda Cl: -(  # noqa E731
                    self.Cms_Re_low_fun(-Cl) * (1.0 - eta)
                    + eta * self.Cms_Re_high_fun(-Cl)
                )
        else:
            alphas = lambda x: 0.0  # noqa E731
            Cls = lambda x: 0.0  # noqa E731
            Cds = lambda x: 0.0  # noqa E731
            Cms = lambda x: 0.0  # noqa E731
        return alphas, Cls, Cds, Cms


def read_polar(
    poldir="", polname="n4412", ext_append=True, echo=True, cubic=True
):  # read polars from dumped file
    ordir = os.getcwd()
    if len(poldir) == 0:
        os.chdir(ordir)
    else:
