...
class aircraft:  # class to ease certain case studies for full aircraft
    ...
    def addmass(
        self, m=0.0, Ixx=0.0, Iyy=0.0, Izz=0.0, Ixy=0.0, Ixz=0.0, Iyz=0.0, echo=True
    ):  # sets mass variables for the aircraft
        if self.sld.runme:
            self.m = m
    ...
    def dynamic_simulation(
        ...
        if self.sld.runme:
            if not self.massavailable:
                print(
                    "WARNING: dynamic analysis requested without any mass data. "
                    "Returning empty arrays"
                )
                return np.array([]), np.array([]), np.array([]), np.array([])
            keys = ["phi", "theta", "psi", "u", "v", "w", "p", "q", "r"]
            for par in keys:
                if par not in perturbations:
            ...
            if not trim:
                coeffs = np.array(
                    [-self.CX, self.CY, -self.CZ, -self.Cl, self.Cm, -self.Cn]
                )  # note that we're using stability coordinates here
                if visc:
                    coeffs += np.array(
                        [-self.dCX, self.dCY, -self.dCZ, -self.dCl, self.dCm, -self.dCn]
                    )
            i = 0
            for par in ["a", "b", "p", "q", "r"]:
                derivs[i, :] = np.array(
                    [
                        -self.stabderivative_dict[par]["dCX"],
                        self.stabderivative_dict[par]["dCY"],
                        -self.stabderivative_dict[par]["dCZ"],
                        -self.stabderivative_dict[par]["dCl"],
                        self.stabderivative_dict[par]["dCm"],
                        -self.stabderivative_dict[par]["dCn"],
                    ]
                )
                i += 1
            (
    ...
    def balance(
        self, SM=0.1, echo=True
    ):  # balancing the aircraft for given static margin
        if self.sld.runme:
            if not self.stabavailable:
                print(
                    "WARNING: stability derivatives not available for balancing. "
                    "Performing in place"
                )
                self.calcstab(echo=echo)
            loc_SM = (
                -self.stabderivative_dict["a"]["dCm"]
                / self.stabderivative_dict["a"]["dCL"]
            )
    ...
    def transp_byvec(
        self, vec
    ):  # transport pre-calculated forces by a 'vec' deallocation in CG
        if self.sld.runme:
    ...
    def __init__(
        ...
        if sld.runme:
            ...
            for e in elems:
                e.set_aircraft(self)
                if isinstance(e, wing):
                    self.wings += [e]
                elif isinstance(e, body):
                    self.bodies += [e]
            # BE VERY AWARE OF THESE WARNINGS WHEN USING
            # FUSELAGE-SEPARATED SEMI-WINGS AS WING OBJECTS
            if len(self.wings) != 0:
            ...
            if Sref == 0.0:
                print(
                    "WARNING: no reference surface provided. "
                    "Using first wing input as reference. "
                    "Address to aircraft constructor's Sref "
                    "argument for better definitions"
                )
                self.Sref = S
            else:
                self.Sref = Sref
            if cref == 0.0:
                print(
                    "WARNING: no reference chord provided. "
                    "Using first wing input as reference. "
                    "Address to aircraft constructor's cref "
                    "argument for better definitions"
                )
                self.cref = mac
            else:
            ...
            if bref == 0.0:
                print(
                    "WARNING: no reference span provided. "
                    "Using first wing input as reference. "
                    "Address to aircraft constructor's bref "
                    "argument for better definitions"
                )
                self.bref = b
            else:
                ...
                for wngqd in wng.wingquads:
                    if wngqd.hascontrol():
                        for cname in wngqd.controls:
                            if cname not in self.controlset:
            ...
            for wng in self.wings:
                for wngqd in wng.wingquads:
                    xkeypts += [
                        wngqd.sect1.CA_position[0] - wngqd.sect1.chord / 4,
                        wngqd.sect1.CA_position[0] + 3 * wngqd.sect1.chord / 4,
                        wngqd.sect2.CA_position[0] - wngqd.sect2.chord / 4,
                        wngqd.sect2.CA_position[0] + 3 * wngqd.sect2.chord / 4,
                    ]
            ...
            for bdy in self.bodies:
                ...
                ykeypts += [
                    bdy.sections[i].center[1] - bdy.sections[i].Rs[i]
                    for i in range(len(bdy.sections))
                ]
                zkeypts += [
                    bdy.sections[i].center[2] - bdy.sections[i].Rs[i]
                    for i in range(len(bdy.sections))
                ]
                ykeypts += [
                    bdy.sections[i].center[1] + bdy.sections[i].Rs[i]
            ...
            if len(xkeypts) != 0:
                ...
                zmin = min(zkeypts)
                self.plotlim = max(
                    [abs(xmax), abs(ymax), abs(zmax), abs(xmin), abs(ymin), abs(zmin)]
                )
            else:
                self.plotlim = 1.0
        else:
    ...
    def addwake(
        ...
        if self.sld.runme:
            # once the wake has been added, one can end geometry
            # pre-processing of panels in question:
    ...
    def calcforces(self, echo=True):
        if self.sld.runme:
            self.CX = (
                sum([self.sld.forces[i][0] for i in range(self.sld.npanels)])
                / self.Sref
            )
            self.CY = (
                sum([self.sld.forces[i][1] for i in range(self.sld.npanels)])
                / self.Sref
            )
            self.CZ = (
                sum([self.sld.forces[i][2] for i in range(self.sld.npanels)])
                / self.Sref
            )
            u = np.array(
                [cos(self.a) * cos(self.b), sin(self.b) * cos(self.a), sin(self.a)]
            )
            v = np.array(
                [-sin(self.a) * cos(self.b), -sin(self.b) * sin(self.a), cos(self.a)]
            )
            adforces = np.array([self.CX, self.CY, self.CZ])
            self.CL = v @ adforces
            self.CD = u @ adforces
            self.Cl = sum([self.sld.moments[i][0] for i in range(self.sld.npanels)]) / (
                self.Sref * self.bref
            )
            self.Cm = sum([self.sld.moments[i][1] for i in range(self.sld.npanels)]) / (
                self.Sref * self.cref
            )
            self.Cn = sum([self.sld.moments[i][2] for i in range(self.sld.npanels)]) / (
                self.Sref * self.bref
            )
            ...
            if self.hascorrections:
                ...
                for wng in self.wings:
                    ...
                    if wng.hascorrection():
                        dCX_wng, dCY_wng, dCZ_wng, dCl_wng, dCm_wng, dCn_wng = (
                            wng.calc_corrected_forces()
                        )
                        self.dCX += dCX_wng
                        self.dCY += dCY_wng
                        self.dCZ += dCZ_wng
    ...
    def freestream_derivative(
        self, par="a"
    ):  # definition of derivative of freestream velocity vectors u and v
        # (normal vectors)
        if par == "a":
            du = np.array(
                [-sin(self.a) * cos(self.b), -sin(self.b) * sin(self.a), cos(self.a)]
    ...
    def calcstab(self, echo=True):
        if self.sld.runme:
            ...
            for p in pars:
                dvdksi = self.sld.gen_farfield_derivative(
                    self.Uinf,
                    a=self.a,
                    b=self.b,
                    p=self.p * 2 * self.Uinf / self.bref,
                    q=self.q * 2 * self.Uinf / self.cref,
                    r=self.r * 2 * self.Uinf / self.bref,
                    par=p,
                )
                dCps = self.sld.calc_derivative_dv(self.Uinf, dvdksi)
                forces = [
                    -dCps[i] * self.sld.panels[i].S * self.sld.panels[i].nvector
                    for i in range(self.sld.npanels)
                ]
                moments = [
                    np.cross(self.sld.panels[i].colpoint, forces[i])
                    for i in range(self.sld.npanels)
                ]
                ...
                v = np.array(
                    [
                        -sin(self.a) * cos(self.b),
                        -sin(self.b) * sin(self.a),
                        cos(self.a),
                    ]
                )
                ...
                dCl = sum([moments[i][0] for i in range(self.sld.npanels)]) / (
                    self.Sref * self.bref
                )
                dCm = sum([moments[i][1] for i in range(self.sld.npanels)]) / (
                    self.Sref * self.cref
                )
                dCn = sum([moments[i][2] for i in range(self.sld.npanels)]) / (
                    self.Sref * self.bref
                )
                self.stabderivative_dict[p] = {
                    "dCX": dCX,
                    "dCY": dCY,
                    "dCZ": dCZ,
                    "dCL": dCL,
    ...
    def stabreport(
        self,
    ):  # report calculated stability derivatives, or calculate them in place
        if self.sld.runme:
            if not self.stabavailable:
                self.calcstab(echo=False)
            print("Stability derivatives")
            print(
                "%5s | %10s %10s %10s %10s %10s %10s %10s %10s"
                % ("par", "dCX", "dCY", "dCZ", "dCL", "dCD", "dCl", "dCm", "dCn")
            )
            for p in self.stabderivative_dict:
                print(
                    "%5s | %10f %10f %10f %10f %10f %10f %10f %10f"
                    % (
                        p,
                        self.stabderivative_dict[p]["dCX"],
                        self.stabderivative_dict[p]["dCY"],
                        self.stabderivative_dict[p]["dCZ"],
                        self.stabderivative_dict[p]["dCL"],
                        self.stabderivative_dict[p]["dCD"],
                        -self.stabderivative_dict[p]["dCl"],
                        self.stabderivative_dict[p]["dCm"],
                        self.stabderivative_dict[p]["dCn"],
                    )
                )
    ...
    def bodies_eqflatplate_apply(
        ...
        if self.sld.runme:
            ...
            for bdy in self.bodies:
                bdy.apply_eqflatplate(
                    rho=rho,
                    mu=mu,
                    turbulent_criterion=turbulent_criterion,
                    Cf_l_rule=Cf_l_rule,
                    Cf_t_rule=Cf_t_rule,
                    Uinf=self.Uinf,
                )
    ...
    def plot_lines(self, ax=None):
        ...
        if ax is None:
            ...
            showNow = True
        ax = plt.axes(projection="3d")
        lines = self.sld.lines
        array_shape = np.shape(lines)
        xlim = []
        ylim = []
        zlim = []
        ...
        for i in range(array_shape[0]):
            segment = [
                ...
                [lines[i, 2, 0], lines[i, 2, 1]],
            ]
            if segment in already_plotted:
                # shift just a bit to make it visible
                segment = np.array(segment) + 0.01
                colour = "red"
            else:
                colour = "blue"
                already_plotted.append(segment)
    ...
    def design_derivatives(
        self, wings=[], echo=True
    ):  # calculate derivatives in forces due to design parameters in a wing
        #  - its sectional incidences
        if self.sld.runme:
            if len(wings) == 0:
                wings = self.wings
            wnglist = []
            ...
            for wng in wings:
                ...
                if echo:
                    print("=============================================")
                    print(
                        "Design derivatives for incidences in wing %d [deg]" % (wngcnt)
                    )
                for sect in range(len(wng.wingquads) + 1):
                    dFs, dMs = wng.calc_design_dFM(
                        section=sect,
                        Uinf=self.Uinf,
                        M=self.M,
                        gamma=self.gamma,
                    ...
                    if echo:
                        ...
                        print(
                            ...
                            % ("dCX", "dCY", "dCZ", "dCD", "dCL", "dCl", "dCm", "dCn")
                        )
                        print(
