...
"""
ROLLPOINT() DIRECTION: IF P2 IS THE RIGHTMOST POINT,
THE FLAP IN QUESTION WILL ROTATE DOWNWARDS (POSITIVE DEFLECTION)
...
def z_rotation_matrix(th):
    return np.array(
        [[cos(th), sin(th), 0.0], [-sin(th), cos(th), 0.0], [0.0, 0.0, 1.0]]
    )
...
class control_axis:
    # definition of control axis with functions to rotate points around it
    ...
    def __init__(self, p0=np.array([0.0, 0.0, 0.0]), p1=np.array([0.0, 1.0, 0.0])):
        ...
        self.control_rot_func = (
            lambda pt, th: Mtouni @ (z_rotation_matrix(-th) @ (Mtouni.T @ (pt - p0)))
            + p0
        )

    def panel_hinge_moment(
        self, pt, F
    ):  # compute hinge moment for a force F applied at point pt
        return self.u @ np.cross(pt - self.p0, F)


class control:
    # main control class, to be summoned attached to a control_DOF
    # instance ran inside aircraft class instance
    def __init__(
        self,
        DOF=None,
    ...
    def panlist(
        self, paninds, sld
    ):  # return indexes of panels belonging to the control surface
        return [
            pan
            for pan in paninds
            if np.cross(sld.panels[pan].colpoint[0] - self.axis.p0, self.axis.u)
            @ np.cross(np.array([1.0, 0.0, 0.0]), self.axis.u)
            > 0.0
        ]
